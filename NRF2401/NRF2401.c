...
uint8_t nrf24l01_readregister(uint8_t reg) {
	...
	SPI_MasterTransmit(NRF24L01_CMD_R_REGISTER | (NRF24L01_CMD_REGISTER_MASK & reg));
    uint8_t result = SPI_MasterTransmit(NRF24L01_CMD_NOP); //read write register
...
void nrf24l01_readregisters(uint8_t reg, uint8_t *value, uint8_t len) {
	...
	SPI_MasterTransmit(NRF24L01_CMD_R_REGISTER | (NRF24L01_CMD_REGISTER_MASK & reg));
	for(i=0; i<len; i++)
		value[i] = SPI_MasterTransmit(NRF24L01_CMD_NOP); //read write register
	nrf24l01_CSNhi; //high CSN
}
...
void nrf24l01_writeregister(uint8_t reg, uint8_t value) {
	nrf24l01_CSNlo; //low CSN
	SPI_MasterTransmit(NRF24L01_CMD_W_REGISTER | (NRF24L01_CMD_REGISTER_MASK & reg));
	SPI_MasterTransmit(value); //write register
	nrf24l01_CSNhi; //high CSN
}
...
void nrf24l01_writeregisters(uint8_t reg, uint8_t *value, uint8_t len) {
    ...
    SPI_MasterTransmit(NRF24L01_CMD_W_REGISTER | (NRF24L01_CMD_REGISTER_MASK & reg));
	for(i=0; i<len; i++)
		 SPI_MasterTransmit(value[i]); //write register
	nrf24l01_CSNhi; //high CSN
}
...
void nrf24l01_flushRXfifo() {
	nrf24l01_CSNlo; //low CSN
	SPI_MasterTransmit(NRF24L01_CMD_FLUSH_RX);
	nrf24l01_CSNhi; //high CSN
}
...
void nrf24l01_flushTXfifo() {
	nrf24l01_CSNlo; //low CSN
	SPI_MasterTransmit(NRF24L01_CMD_FLUSH_TX);
	nrf24l01_CSNhi; //high CSN
}
...
uint8_t nrf24l01_getstatus() {
	uint8_t status = 0;
	nrf24l01_CSNlo; //low CSN
	status = SPI_MasterTransmit(NRF24L01_CMD_NOP); //get status, send NOP request
	nrf24l01_CSNhi; //high CSN
	return status;
}
...
void nrf24l01_read(uint8_t *data) {
	...
	//read rx register
	nrf24l01_CSNlo; //low CSN
    SPI_MasterTransmit(NRF24L01_CMD_R_RX_PAYLOAD);
    for(i=0; i<NRF24L01_PAYLOAD; i++)
    	data[i] = SPI_MasterTransmit(NRF24L01_CMD_NOP);
    nrf24l01_CSNhi; //high CSN
    //reset register
...
uint8_t nrf24l01_write(uint8_t *data) {
	...
	//write data
	nrf24l01_CSNlo; //low CSN
	SPI_MasterTransmit(NRF24L01_CMD_W_TX_PAYLOAD);
	for (i=0; i<NRF24L01_PAYLOAD; i++)
